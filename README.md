...
A simple Gradio app that sends stateless prompts to **Gemini 3.1 Pro Preview** (via OpenRouter) or **Claude Opus 4.6** and logs every interaction to **Supabase**. Optionally syncs to a live Google Sheet via a Supabase database webhook.
...
```bash
git clone https://github.com/<your-username>/text-response-gen-ui.git
...
| `SUPABASE_URL` | Supabase dashboard > Project Settings > General > Project ID (URL is `https://<project-id>.supabase.co`) |
...
You can auto-sync every new row to a Google Sheet using a Supabase database webhook:
